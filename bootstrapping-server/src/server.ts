...
// Usable as a module (createSignalingServer) or run directly (node dist/server.js).
...
import type {
  ...
  SignalingServerSettings,
  SignalingStats,
  SocketState,
} from './types.js';

export const DEFAULTS: SignalingServerSettings = {
  host: '127.0.0.1',
  port: 8788,
...
export function createSignalingServer(userOptions: SignalingServerOptions = {}): SignalingContext {
  const options: SignalingServerSettings = {
    ...DEFAULTS,
    ...userOptions,
    rateLimit: { ...DEFAULTS.rateLimit, ...(userOptions.rateLimit ?? {}) },
  };
  // Global FIFO of waiting clients.
  const queue: SocketState[] = [];
  const states = new Set<SocketState>();
  const send = (state: SocketState, payload: LobbyServerMessage): void => {
    if (state.ws.readyState === state.ws.OPEN) state.ws.send(JSON.stringify(payload));
  };
  const sendError = (state: SocketState, code: string): void => send(state, { t: 'error', code });

  function stats(): SignalingStats {
    let paired = 0;
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (data: RawData) => handleIncoming(state, data));
    ws.on('close', () => handleDisconnected(state));
    ws.on('error', () => { /* close handler cleans up */ });
  });

  function sigRateLimited(state: SocketState): boolean {
  ...
  function handleIncoming(state: SocketState, data: RawData): void {
    ...
    const frame = message as { t?: unknown };
    switch (frame.t) {
      case 'find':
      ...
      case 'sig':
        ...
        if (state.partner) send(state.partner, { t: 'sig', data: (frame as { data?: unknown }).data });
        return;
      default:
  ...
  function handleFind(state: SocketState): void {
  ...
  function handleCancel(state: SocketState): void {
  ...
  function handleDisconnected(state: SocketState): void {
    states.delete(state);
